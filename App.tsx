...
const App: React.FC = () => {
  ...
  const refreshUserCredits = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/user?${query.toString()}`, { headers });
                      ...
                      <div
                        className={`absolute inset-x-8 -bottom-6 h-16 rounded-full blur-3xl pointer-events-none transition-all duration-500 ${
                          promptGlowState === 'success'
                            ? 'bg-[linear-gradient(90deg,rgba(244,114,182,0.26)_0%,rgba(251,191,36,0.22)_24%,rgba(74,222,128,0.18)_50%,rgba(56,189,248,0.20)_74%,rgba(168,85,247,0.24)_100%)] [background-size:200%_100%] animate-rainbow opacity-100'
                            : isGeneratingPrompt
                      ...
                      <div
                        className={`absolute inset-x-6 -bottom-7 h-24 rounded-full blur-3xl pointer-events-none transition-all duration-500 ${
                          copyGlowState === 'success'
                            ? 'bg-[linear-gradient(90deg,rgba(244,114,182,0.28)_0%,rgba(251,191,36,0.22)_24%,rgba(74,222,128,0.20)_50%,rgba(56,189,248,0.22)_74%,rgba(168,85,247,0.24)_100%)] [background-size:200%_100%] animate-rainbow opacity-100'
                            : isExtractingCopy
            ...
            {!isSuiteMode && isProcessing && resultImages.length === 0 ? (
              <div className="w-full space-y-12">
                ...
                <div className="w-full max-w-3xl mx-auto">
                  <div className="render-canvas render-canvas-neutral relative w-full rounded-[2rem] shadow-[0_12px_36px_rgba(15,23,42,0.06)] flex flex-col items-center justify-center group" style={{ aspectRatio: currentAspectRatio, minHeight: 320 }}>
                      ...
                      return (
                        ...
                        >
                          ...
                          {img ? (
                            ...
                            <div className="render-canvas relative w-full h-full rounded-[32px] flex flex-col items-center justify-center">
                              <span className="render-status-text relative z-20 text-[12px] font-mono text-gray-500 tracking-[0.18em] uppercase [text-shadow:0_1px_10px_rgba(255,255,255,0.7)]">
                                Retry Needed
                              </span>
